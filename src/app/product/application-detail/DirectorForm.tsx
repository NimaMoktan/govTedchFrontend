...
const HeadForm = ({ applicationNumber, appDetails }: { 
    ...
    const handleSubmit = async (values: any) => {
        ...
        try {
            const claimApp = {
                ...
                parameter: "defaultParameterValue" // Replace with the appropriate value
            };

            const response = await updateWorkFlow(claimApp);
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.currentTarget.files && event.currentTarget.files.length > 0) {
            ...
            if (allowedTypes.includes(selectedFile.type)) {
                ...
                event.currentTarget.value = ''; // Reset the file input
            }
        }
    };
                ...
                validationSchema={Yup.object({
                    ...
                    file: Yup.mixed()
                        .test('fileType', 'Only Excel files are accepted', (value) => {
                            if (!value) return true; // Optional if you want to make it required
