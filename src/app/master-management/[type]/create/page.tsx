...
import { getParentMastersByType, createMaster } from "@/services/master/MasterService";
import { toast } from "react-toastify";
import { useRouter } from "next/navigation";

const MasterCreatePage = ({params}: { params: Promise<{ type: string }>}) => {
    ...
    const handleSubmit = async (values: any, { setSubmitting }: any) => {
        try {
            await createMaster(values).then((response) => {
                toast.success(response.data.message)
                setTimeout(() => {
            router.push(`/master-management/${param.type}`);
        }, 1000);
            }).finally(() => setSubmitting(false)).catch((error) => {
    ...
    return (
         <DefaultLayout>
            <Breadcrumb pageName={`Create ${(param.type).charAt(0).toUpperCase() + (param.type).slice(1)}`} parentPage='Master Management' />
