...
const MasterPage = ({params}: { params: Promise<{ type: string }>}) => {
    ...
    const handleDelete = async (master: Master) => {
        ...
        await deleteMaster(master.id).then(() => {
            Swal.fire({
                title: 'Deleted!',
                ...
                setMasterList((prevMasters) => prevMasters.filter((m) => m.id !== master.id));
            });
        }).catch((error) => {
    ...
    useEffect(()=>{
        ...
        const getMasters = async() => {
            setIsLoading(true)
            const response = await getMastersByType(param.type).finally(() => setIsLoading(false));
    ...
    return (
        <DefaultLayout>
            <Breadcrumb pageName={`${(param.type).charAt(0).toUpperCase() + (param.type).slice(1)} Master List`}  parentPage='Master Management' />
