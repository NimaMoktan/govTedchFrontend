...
const SampleTestTable = () => {
    ...
    const fetchProducts = useCallback(async () => {
        try {
            const response = await fetch('http://172.31.1.80:8081/sampleTestType/', {
                method: 'GET',
                headers: {
    ...
    const handleDelete = async (id: number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const response = await fetch(`http://172.31.1.80:8081/sampleTestType/${id}/delete`, {
                        method: 'POST',
                        headers: {
                            'Authorization': `Bearer ${token}`,
                        },
    ...
    const handleSubmit = async (values: Omit<Service, 'id'>, { resetForm }: { resetForm: () => void }) => {
        try {
            ...
            if (selectedProduct) {
                ...
                response = await fetch(`http://172.31.1.80:8081/sampleTestType/${selectedProduct.id}/update`, {
                ...
                response = await fetch('http://172.31.1.80:8081/sampleTestType/', {
                ...
                <table className="w-full border-collapse">
                    ...
                    <tbody>
                        {Array.isArray(products) && products.length > 0 ? (
                            ...
                            <tr>
                                <td colSpan={6}>No sample available</td>
                            </tr>
                        )}
                    </tbody>
                </table>
