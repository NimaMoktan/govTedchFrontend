...
const ServiceTypeTable = () => {
    ...
    const fetchProducts = useCallback(async () => {
        try {
            const response = await fetch('http://172.31.1.80:8081/status/', {
                method: 'GET',
                headers: {
    ...
    const handleDelete = async (id: number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const response = await fetch(`http://172.31.1.80:8081/status/${id}/delete`, {
                        method: 'POST',
                        headers: {
                            'Authorization': `Bearer ${token}`,
                        },
                    ...
                    if (response.ok) {
                        ...
                        Swal.fire("Error!", "Failed to delete the status. Please try again.", "error");
                    }
                } catch (error) {
                    console.error("Error deleting status:", error);
                    Swal.fire("Error!", "An error occurred while deleting the status. Please try again.", "error");
                }
            }
        });
    };    
    ...
    const handleSubmit = async (values: Omit<Service, 'id'>, { resetForm }: { resetForm: () => void }) => {
        try {
            ...
            if (selectedProduct) {
                ...
                response = await fetch(`http://172.31.1.80:8081/status/${selectedProduct.id}/update`, {
                ...
                response = await fetch('http://172.31.1.80:8081/status/', {
            ...
            Swal.fire({
                icon: "error",
                title: "Error",
                text: "Duplicate status code detected. Please try again.",
            });
            closeModal();
            resetForm();
        }
